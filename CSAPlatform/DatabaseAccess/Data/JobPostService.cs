...
namespace DatabaseAccess.Data
{
    public class JobPostService
    {
        ...
        {
            var jobList = _context.JobPosts.Include(job => job.User).ToList();
            return jobList;
        }

        public async Task<List<JobPost>> GetUserJobPosts(string userId)
        {
            var jobList = _context.JobPosts.Include(job => job.User).Where(x => x.User.Id == userId).ToList();
            return jobList;
        ...
        {
            JobPost job = _context.JobPosts.Include(job => job.User).FirstOrDefault(x => x.Id == id);
            return job;
